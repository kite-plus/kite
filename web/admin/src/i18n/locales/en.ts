const en = {
  ...
  auth: {
    ...
    signInDesc: 'Sign in to your Kite account',
    createAccount: 'Create account',
    createAccountDesc: 'Fill in the details to start your Kite journey',
    ...
    chooseUsername: 'Enter a username',
    ...
    passwordMismatch: 'Passwords do not match — please retype',
    registrationFailed: 'Registration failed, please try again later',
    registrationClosed: 'Registration is closed on this site',
    registrationClosedDesc:
      'To create an account, please contact an administrator or try again later.',
    profile: 'Account Center',
    loggedOut: 'Signed out',
    // The bilingual aria-label and document-title strings used to be
    // hard-coded — keeping them in the catalogue means the public
    // header switcher and admin language picker flip everything in
    // lockstep, including assistive-tech hints.
    themeToggleAria: 'Toggle theme',
    adminDocumentTitle: 'Kite admin',
    // Sign-in form scaffolding.
    signInSuccess: 'Signed in!',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    continueWith: 'Or continue with',
    // Two-factor TOTP step. The TOTP form replaces the password form
    // when the server returns a challenge token; these keys describe
    // the prompt + buttons rendered while the user enters the code.
    totpTitle: 'Two-factor verification',
    // The "{username}" placeholder is replaced at runtime so we can
    // render `Account <bold>alice</bold> has 2FA enabled.` without
    // splitting the JSX into three nodes.
    totpHint:
      'Account {username} has 2FA enabled. Enter the 6-digit code from your authenticator.',
    totpLabel: 'Code',
    verifying: 'Verifying...',
    verifyAndLogin: 'Verify and sign in',
    // Status-aware toast errors. The login flow inspects the HTTP
    // status code and picks the matching key, falling back to
    // errLoginFailed when no specific match.
    errLoginFailed: 'Sign in failed, please try again later',
    errAccountDisabled: 'Account disabled, please contact an administrator',
    errRateLimited: 'Too many attempts, please try again later',
    errServerUnavailable: 'Service is temporarily unavailable',
    errTotpInvalid: 'Verification failed, please retry',
    errTotpExpired: 'Verification session expired, please sign in again',
    // Footer + terms acknowledgements. Splitting the sentence into
    // pre/link/middle/link/suffix keys keeps the inline anchor markup
    // out of the locale files (translators just see plain text).
    ...
    termsSuffix: '.',
    // OAuth callback (`/login/callback?ticket=…`). Renders a spinner
    // while we exchange the ticket for cookies; the toast key fires
    // when the ticket is missing or the exchange fails.
    callbackTitle: 'Completing sign-in',
    callbackSubtitle: 'Just a moment, we are taking you to Kite.',
    errTicketMissing: 'Sign-in ticket is missing, please try again',
    errOAuthFailed: 'Third-party sign-in failed, please try again later',
    // Social-onboarding page (`/complete-social`). Shown when an
    // OAuth provider didn't return a usable email so we need a
    // username + email from the user before creating the account.
    completeTitle: 'Complete your profile',
    completeSubtitle:
      'This third-party sign-in did not return a usable email — please fill in a username and email to continue.',
    completeFinish: 'Finish and enter Kite',
    completeSuccess: 'Account created — welcome to Kite!',
    completeFailed: 'Profile completion failed, please try again later',
    completeMissingFields: 'Please fill in both username and email',
    completeRegistrationClosedTitle: 'Registration is currently closed',
    completeRegistrationClosedDesc:
      'This site is not accepting new registrations. Ask an administrator to provision an account before linking your social login.',
    // Hero panel rendered to the right of the auth form on desktop.
    heroSlogan: 'Soar',
    heroTagline:
      'Manage your files, albums and assets with the lightness of a kite — storage that lifts up and stays within reach.',
    feature1Title: 'Unified file management',
    feature1Desc:
      'Centralise multiple storage backends — control every asset in one place.',
    feature2Title: 'Smart album organisation',
    feature2Desc:
      'Albums and thumbnails arranged automatically for effortless browsing.',
    feature3Title: 'Fine-grained access control',
    feature3Desc:
      'Users, tokens and quotas in one console — secure by default.',
    // Brand label for the inline social-provider logo. GitHub and Google
    // are spelled the same way everywhere, so they stay hard-coded. WeChat
    // is the only provider whose brand reads differently across locales.
  ...
  // File-type label strings shown next to the icon in file lists.
  // The key naming follows the extMap entries in lib/file-utils.ts —
  // proper-noun keys (pdf, word, excel) are kept English-only because
  // brand names don't translate; the descriptive nouns (document,
  // spreadsheet, archive) get a Chinese counterpart.
  fileType: {
    pdf: 'PDF document',
    word: 'Word document',
    odt: 'ODT document',
    ...
    tgzArchive: 'TGZ archive',
    bz2Archive: 'BZ2 archive',
    xzArchive: 'XZ archive',
    ...
    debInstaller: 'DEB package',
    rpmInstaller: 'RPM package',
    apkInstaller: 'APK package',
    ipaInstaller: 'IPA package',
    jarPackage: 'JAR package',
    warPackage: 'WAR package',
    photoshop: 'Photoshop',
