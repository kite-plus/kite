 ...
 * the themes in themes/ and the pictures settings name. They belong to no
...
export function PublishBar() {
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-lg border border-warning/30 bg-warning/5 px-4 py-2.5 text-sm lg:mb-6">
...
/** settingsPaths are the uncommitted files the settings screens wrote, a theme named by its folder. */
function settingsPaths(dirty: string[]): string[] {
  const out = new Set<string>();
  for (const path of dirty) {
    if (path === "kite.yaml") out.add(path);
    else if (path.startsWith("themes/")) out.add(path.split("/").slice(0, 2).join("/"));
