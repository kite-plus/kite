...
export function Page({ title, description, actions, toolbar, children }: Props) {
  return (
    <div className="flex h-svh min-w-0 flex-col">
      ...
      <header className="flex flex-wrap items-start justify-between gap-x-4 gap-y-3 px-4 pt-5 pb-3.5 sm:px-7">
      ...
      {toolbar && <div className="px-4 pb-3.5 sm:px-7">{toolbar}</div>}
      <div className="min-h-0 flex-1 overflow-auto px-4 pb-6 sm:px-7">{children}</div>
    </div>
  );
}
