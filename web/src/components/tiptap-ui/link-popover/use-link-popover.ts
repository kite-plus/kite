...
export interface UseLinkPopoverConfig {
  ...
  onSetLink?: () => void
  /**
   * Kite: turns a link as written into one the browser can open, since a
   * link relative to the page means nothing relative to the admin.
   */
...
export function useLinkHandler(props: LinkHandlerProps) {
  const { editor, onSetLink, resolveUrl } = props
