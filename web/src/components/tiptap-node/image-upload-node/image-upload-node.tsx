...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    // Kite: a maxSize of 0 means no limit, as the option documents.
    if (options.maxSize > 0 && file.size > options.maxSize) {
      const error = new Error(
        t("editor.upload.tooLarge", {
          name: file.name,
  ...
  const uploadFiles = async (files: File[]): Promise<string[]> => {
    ...
    if (options.limit && files.length > options.limit) {
      options.onError?.(
        new Error(t("editor.upload.tooMany", { count: options.limit }))
      )
      return []
    }
      ...
      <div className="tiptap-image-upload-preview-content">
        ...
        <div className="tiptap-image-upload-actions">
          ...
          <Button
            type="button"
            variant="ghost"
            aria-label={t("editor.upload.cancel")}
            onClick={(e) => {
              e.stopPropagation()
              onRemove()
            }}
...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
  ...
  const { t } = useI18n()
  // One sentence per language, with the words to click set inside it.
  const [before, after] = t("editor.upload.prompt").split("{click}")
  return (
    <>
      ...
      <div className="tiptap-image-upload-content">
        <span className="tiptap-image-upload-text">
          {before}
          <em>{t("editor.upload.click")}</em>
          {after}
        </span>
        <span className="tiptap-image-upload-subtext">
          {t("editor.upload.limit", {
            count: limit,
  ...
  const handleUpload = async (files: File[]) => {
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        // Kite: the image keeps only what markdown writes, src and alt;
        // a title would repeat the file name in every image's markdown.
        const imageNodes = urls.map((url, index) => {
          const filename =
            files[index]?.name.replace(/\.[^/.]+$/, "") || ""
          return {
            type: extension.options.type,
            attrs: { src: url, alt: filename },
          }
        })
        ...
        <div className="tiptap-image-upload-previews">
          {fileItems.length > 1 && (
            <div className="tiptap-image-upload-header">
              <span>
                {t("editor.upload.uploading", { count: fileItems.length })}
              </span>
              <Button
                type="button"
                variant="ghost"
