...
填好站点名称、地址和语言就在这里建好站点，问的就是 `kite init` 在终端里问的那些。
...
| **设置** | 站点的标题、描述、地址、语言、作者、关键词、时区、搜索引擎收录和全站附加代码；后台自己的语言和配色；你的账户：资料、头像、密码和登录会话 |
...
同一个页面里还有个人资料：显示名称、邮箱和头像，显示在后台的菜单里。它们和账号
放在一起，存在 `.kite/secrets/profile.json` 和 `.kite/secrets/avatar`，永远不会
发布；主题显示的作者是 `site.author`。
