...
它用这套主题构建一个用到每种页面的小站点，再向服务器请求同样的页面，逐字节比较。
通过检查的主题，发布出去的就是 `kite run` 预览时看到的；没通过的，会指出每一页第一
处不同的行。不给目录时，检查当前项目在用的主题，在项目之外则检查内置主题。
...
定时文章由另一个工作流 `scheduled.yml` 发布。每次构建都会记下下一篇定时文章的时间，
这个工作流每小时检查一次，时间已过才部署，所以定时文章会在设定时间之后的一小时内
上线，没有文章到点的那一小时只跑一个很短的检查。私有仓库里每次检查按 1 分钟的
Actions 时长计费，想少查几次，改它的 `cron` 一行即可。
