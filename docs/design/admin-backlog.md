...
| 分类与标签 | 每种词条一个页面（侧边栏的“分类”“标签”），表格可搜索、按文章数排序，按词条进入内容列表或站点页面 | 没有跨文章重命名或合并词条；词条本身从内容推导，不宜做独立 CRUD |
| 主题与设置 | 主题列表、上传 zip 安装与删除、切换；自定义器左边是由 schema 生成的设置，右边是整站实时预览；主题文字按界面语言显示；站点基本设置、界面语言与外观；改过的 `kite.yaml` 和主题文件在设置页顶部直接发布 | 还不能从 Git 地址安装主题；菜单还没写进主题契约 |
| 系统 | 单账号登录、账户设置（资料、头像、密码、会话）、安装引导、发布状态 | 评论、附件库、菜单、插件、备份入口尚未实现；只读服务器仍展示可写操作 |
...
| A06 | P1 | 已完成 | **补齐错误态和重试入口。** 仪表盘和分类法主要依据 `data` 判断加载，查询失败时可能持续显示骨架；主题/设置页虽有错误提示，表单仍可能保持骨架。区分加载、无数据、失败，提供重试并保留旧数据。验收：进入后台后让某个页面的业务请求失败，该页面出现可理解的错误和重试按钮，恢复后正常显示。 | 页面出错时错误页显示在布局内（`web/src/features/errors/page-error.tsx`）；仪表盘各卡片、列表和词条页读取失败时显示“读取失败 + 重试”（`web/src/components/query-error.tsx`），已有数据在重新读取失败时保留；设置页读取失败时提供重新加载 |
...
| 功能 | 建议范围与前置决策 | 阶段 |
|---|---|---|
| 菜单、从 Git 地址安装主题 | 定义 `kite.yaml` 菜单并写进主题契约；Git 安装复用 zip 安装的写入（`PutTheme`）和检查。 | M5 |
| 全站附件库、仪表盘上传 | 建索引、引用关系和清理规则，再做上传、检索、复用。 | v1.5 媒体库 |
| 评论与审核 | 静态站点先决定是否接外部评论组件；Kite 自存评论与审核需要动态存储或插件。 | 集成方案待定；自存 M7/M8 |
| 可见性、备份 | 文章加密、私密内容与备份/恢复一起设计，只在服务端部署提供。不做多用户：博客只有一个站长，后台只有一个账号（2026-09-26 定）。 | M7 |
